...
int main() {
    int N, M, book, cost;
    cin>>N>>M;
    vector<vector<int>> capacity(M+1,vector<int>(N+1));
    MCMF mcmf(N+M+2,0,N+M+1);
    for(int i=1;i<=N;i++) {
        cin>>book;
        mcmf.add_edge_to_sink(M+i,book,0);
    }
    for(int j=1;j<=M;j++) {
        cin>>book;
        mcmf.add_edge_from_source(j,book,0);
    }
    for(int i=1;i<=M;i++) {
        for(int j=1;j<=N;j++) {
            cin>>capacity[i][j];
    ...
    for(int i=1;i<=M;i++) {
        for(int j=1;j<=N;j++) {
            cin>>cost;
            mcmf.add_edge(i,M+j,capacity[i][j],cost);
        }
    }
    auto answer = mcmf.flow();
