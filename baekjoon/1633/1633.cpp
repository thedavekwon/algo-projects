...
int main() {
    ...
    MCMF mcmf(a.size()+4,0,a.size()+3);
    mcmf.add_edge_from_source(1,15,0);
    mcmf.add_edge_from_source(2,15,0);
    for(int i=3;i<=a.size()+2;i++) {
        mcmf.add_edge(1,i,1,-a[i-3].first);
        mcmf.add_edge(2,i,1,-a[i-3].second);
    }
    for(int i=3;i<=a.size()+2;i++) {
