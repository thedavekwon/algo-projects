...
int main() {
        ...
        for(int j=0;j<N;j++) {
            if(In[i] < Out[j]) {
                ...
                if(diff < T) {
                    ...
                    mcmf.add_edge(i+1,N+j+1,1,penalty);
                } else {
                    mcmf.add_edge(i+1,N+j+1,1,0);
                }
            }
        ...
        for(int j=0;j<N;j++) {
            if(In[i] < Out[j]) {
                ...
                if(diff < T) {
                    ...
                    mcmf2.add_edge(i+1,N+j+1,1,-penalty);
                } else {
                    mcmf2.add_edge(i+1,N+j+1,1,0);
                }
            }
    ...
    for(int i=N+1;i<=2*N;i++) {
        mcmf2.add_edge_to_sink(i,1,0);
    }
    cout<<" "<<-mcmf2.flow().second<<endl;
}